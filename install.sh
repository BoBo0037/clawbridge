...
if [[ "$ENABLE_TUNNEL" =~ ^[Yy]$ ]] || [ "$USE_VPN" = true ]; then
    # If VPN found, we still setup CF tunnel as backup/dual-stack unless --no-tunnel was explicit
    # But user asked: "If found... direct use... if none then CF"
    # Actually user said: "If detected... directly use... generate link. If none, go CF logic."
    
    # Logic:
    # 1. If VPN found -> Skip cloudflared download/setup. Just use VPN IP.
    # 2. If NO VPN -> Do Cloudflare logic.
    
    if [ "$USE_VPN" = true ] && [ -z "$TOKEN" ]; then
...
if [ "$QUICK_TUNNEL" = true ] || [ -z "$CF_TOKEN" ]; then
    # ONLY if VPN is NOT used (Branching logic)
